...
${contactInfo.firstName} ${contactInfo.lastName}`)
  ...
  const saveCurrentBooking = async () => {
    if (!user || tourSelections.length === 0) return
    ...
    try {
      const bookingData = {
        ...
        tour_name: tourSelections.map((t) => `${t.tourType} - ${REGION_DATA[t.bioregion]?.shortName}`).join(", "),
        tour_type: tourSelections[0]?.tourType || "Adventure Tours",
        start_date: tourSelections[0]?.startDate?.toISOString().split("T")[0],
        end_date: tourSelections[tourSelections.length - 1]?.endDate?.toISOString().split("T")[0],
    ...
    <div className="min-h-screen bg-gradient-to-br from-emerald-50 to-blue-50">
      ...
      {user && (
        <UserDashboard
          ...
          onSaveBooking={saveCurrentBooking}
        />
      )}
